...
def download_kaggle_dataset(dataset: str):
    ...
    data_path = config.DATA_PATH / dataset
    if data_path.exists():
        print(f"Dataset already exists in {data_path}")
        print("Skipping download.")
    else:
        os.system(f"kaggle datasets download -d {user + '/' + dataset} -p {config.DATA_PATH}")
...
def download_kaggle_competition(competition: str):
    ...
    data_path = config.DATA_PATH / competition
    if data_path.exists():
        print(f"Dataset already exists in {data_path}")
        print("Skipping download.")
    else:
        os.system(f"kaggle competitions download -c {competition} -p {config.DATA_PATH}")
        os.system(f"unzip {data_path}.zip -d {data_path} > /dev/null")
        os.system(f"rm {data_path}.zip")
