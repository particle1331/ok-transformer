...
# The following notebook is meant to give a short introduction to PyTorch basics, and get you setup for writing your own neural networks. **PyTorch** is an open source machine learning framework that allows you to write your own neural networks and optimize them efficiently. However, PyTorch is not the only framework of its kind. Alternatives to PyTorch include TensorFlow, JAX and MXNet. One reason to choose PyTorch is because it is well established, has a huge developer community, is very flexible, and especially used in research. Many current papers publish their code in PyTorch, and thus it is good to be familiar with PyTorch as well.
...
# ```
# 
# As in every machine learning framework, PyTorch provides functions that are stochastic like generating random numbers. However, a very good practice is to setup your code to be reproducible with the exact same random numbers by setting the random seed.
# 
...
# ```
# 
# Tensors are the PyTorch equivalent to NumPy arrays, with the addition to also have support for GPU acceleration (more on that later). In this case, the name "tensor" is a generalization of concepts you already know. For instance, a vector is a 1-D tensor, and a matrix a 2-D tensor. When working with neural networks, we will use tensors of various shapes and number of dimensions. Most common functions you know from NumPy can be used on tensors as well.
...
# You can obtain the shape of a tensor using `x.shape` (same as NumPy) or using the `.size` method:
...
# ### Gradient tracking on computational graphs
# 
# Given an input $\mathbf{x}$, we define our function by a series of computations on that input, usually by matrix-multiplications with weight matrices, additions with so-called bias vectors, and transformations by nonlinear functions. As we manipulate our input, we are automatically creating a computational graph. This graph shows how to arrive at our output from our input. 
# 
# One of the main reasons for using PyTorch in deep learning projects is that we can automatically get gradients or partial derivatives of functions that we define.More precisely, PyTorch is a define-by-run framework; this means that we can just do our manipulations, and PyTorch will keep track of that graph for us. The only thing we have to do is to compute the output, creating a dynamic computation graph along the way, and then we can ask PyTorch to automatically get the gradients. Recall that these gradients will be used to update weights using variants of SGD.

# In[ ]:


# By default gradient tracking is off
x = torch.ones((3,))
print(x.requires_grad)


# ```{margin}
# **Tensors = numpy arrays + GPU support + gradient tracking**
# ```

# For an existing tensor, this can be changed using the function `requires_grad_()` (underscore indicating that this is an in-place operation). Alternatively, when creating a tensor, you can pass the argument `requires_grad=True` to most initializers we have seen above.
...
# ```
# 
# A crucial feature of PyTorch is the support of GPUs, short for Graphics Processing Unit. A GPU can perform many thousands of small operations in parallel, making it very well suitable for performing large matrix operations in neural networks. When comparing GPUs to CPUs, we can list the following main differences [[K. Krewell (2009)]](https://blogs.nvidia.com/blog/2009/12/16/whats-the-difference-between-a-cpu-and-a-gpu/):
# 
...
# To a tensor to the GPU, we use either `.to("cuda")`, or `.cuda()`. Now let's create a tensor and push it to the device:
...
# In case you have a GPU, you should now see the attribute `device='cuda:0'` being printed next to your tensor. The zero next to cuda indicates that this is the zeroth GPU device on your computer. PyTorch also supports multi-GPU systems, but this you will only need once you have very big networks to train (this use-case is covered in the [PyTorch docs](https://pytorch.org/docs/stable/distributed.html#distributed-basics)). 
# 
# ```{margin}
# **Timing matmul on the GPU**
# ```
# 
...
# ```
# 
# We will introduce the libraries and all additional parts you might need to train a neural network in PyTorch, using a simple example classifier on a simple yet well known example: XOR. Given two binary inputs ${x}_1$ and ${x}_2$, its label is 1 if $x_1 \neq x_2,$ otherwise 0. The example became famous by the fact that a single neuron, i.e. a linear classifier, cannot learn this simple function. Hence, we will learn how to build a small neural network that can learn this function. To make it a little bit more interesting, we move the XOR into continuous space and introduce some gaussian noise on the binary inputs. Our desired separation of an XOR dataset could look as follows:
# 
