...
def download_kaggle_dataset(dataset: str):
    ...
    os.system(f"unzip {config.DATASET_DIR / dataset}.zip -d {config.DATASET_DIR / dataset} > /dev/null")
...
def download_kaggle_competition(competition: str):
    ...
    os.system(f"unzip {config.DATASET_DIR / competition}.zip -d {config.DATASET_DIR / competition} > /dev/null")
